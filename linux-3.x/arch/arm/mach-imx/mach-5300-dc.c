/*
 * Copyright 2013 Greg Ungerer, <greg.ungerer@accelecon.com>
...
static struct class *accelecon_class;
...
{
	accelecon_class = class_create(THIS_MODULE, "accelecon");
	if (IS_ERR(accelecon_class))
		return;

	sensor_dev = device_create(accelecon_class, NULL, 0, NULL, "sensor");
	if (IS_ERR(sensor_dev))
		sensor_dev = NULL;

	/* if nothing is using the class, destroy it */
	if (!sensor_dev) {
		class_destroy(accelecon_class);
		accelecon_class = NULL;
	}
}
...
/*
 * We use a dynamic root filesystem partition size. This minimizes wasted
 * flash space, and still puts the kernel at a fixed offset into flash.
 * Also need to allow for the dial images stored in flash.
 */
static int uksize;
core_param(uksize, uksize, int, 0400);

static void dcd_adjust_parts(int p)
{
	if ((uksize < FLASH_KERNEL_MIN) || (uksize > FLASH_KERNEL_MAX))
		return;

	uksize = (uksize + 0x10000 - 1) & 0xffff0000;
	dcd_partitions[p].size = uksize;
	dcd_partitions[p + 1].offset = dcd_partitions[p].offset + uksize;
	dcd_partitions[p + 1].size = dcd_partitions[p + 2].size - uksize;
}

 ...
 *
 *  Copyright (c) 2013, Greg Ungerer <greg.ungerer@accelecon.com>
 *
...
#include "../../arch/arm/mach-imx/devices-imx25.h"
#include "../../arch/arm/mach-imx/hardware.h"
#include "../../arch/arm/mach-imx/mx25.h"
...
static void si32260_ssi_init(void)
{
	struct clk *clkp;

	clkp = clk_get_sys("imx-ssi.0", NULL);
	if (IS_ERR(clkp))
		printk(KERN_ERR DRV_NAME ": failed to get imx-ssi.0 ipg clk\n");
	else
		clk_prepare_enable(clkp);

	clkp = clk_get_sys("imx-ssi.0", "per");
	if (IS_ERR(clkp))
		printk(KERN_ERR DRV_NAME ": failed to get imx-ssi.0 per clk\n");
	else
		clk_prepare_enable(clkp);

	ssip = ioremap(MX25_SSI1_BASE_ADDR, SZ_16K);
	if (ssip == NULL) {
...
static void si32260_audmux_init(void)
{
	struct clk *clkp;

	clkp = clk_get_sys(NULL, "audmux");
	if (IS_ERR(clkp))
		printk(KERN_ERR DRV_NAME ": failed to get audmux clk\n");
	else
		clk_prepare_enable(clkp);

	audmuxp = ioremap(MX25_AUDMUX_BASE_ADDR, SZ_16K);
	if (audmuxp == NULL) {
...
static const struct spi_device_id si32260_ids[] = {
	{ "si32260", 0 },
	{ },
};

static struct spi_driver si32260_driver = {
	.driver = {
		.name = "si32260",
		.owner = THIS_MODULE,
	},
	.id_table = si32260_ids,
	.probe = si32260_probe,
	.remove = si32260_remove,
};

module_spi_driver(si32260_driver);
...
{
	extern void si32260_reset(int state);
...
static int __init si32260_init(void)
{
	...
	si32260_fiq_init();
	mxc_set_irq_fiq(MX25_INT_SSI1, 1);
...
{
	disable_fiq(MX25_INT_SSI1);
	si32260_ssi_intdisable();
	mxc_set_irq_fiq(MX25_INT_SSI1, 0) ;
